...
const fetchPlantas = async () => {
    try {
        ...
        showNotification('Falha ao carregar dados da API.', 'error');
    }
};
...
const savePlanta = async (event) => {
    ...
    try {
        ...
        if (!response.ok) throw new Error(`Erro ao ${id ? 'alterar' : 'incluir'} planta`);
        
        showNotification(`Planta ${id ? 'alterada' : 'incluída'} com sucesso!`, 'success');
        clearForm();
        fetchPlantas();
    } catch (error) {
        console.error(`Falha ao salvar planta:`, error);
        showNotification('Não foi possível salvar a planta.', 'error');
    }
};
...
const deletePlanta = async (id) => {
    ...
    try {
        ...
        showNotification('Planta deletada com sucesso!', 'success');
        fetchPlantas();
    } catch (error) {
        console.error(`Falha ao deletar planta:`, error);
        showNotification('Não foi possível deletar a planta.', 'error');
    }
};
...
const clearForm = () => {
    plantaIdInput.value = '';
    form.reset();
    formTitle.textContent = 'Adicionar Nova Planta';
};
...
const prepareEdit = (planta) => {
    ...
    formTitle.textContent = `Editando: ${planta.nome_popular}`;
